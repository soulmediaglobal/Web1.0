...
import { siteContentKeys, type CaseStudy, type ContentSnapshot, type Leader, type SiteContent, type SiteContentKey, type Solution } from './types'
...
type LeaderRow = { id: string; number: string; name: string; role: string; description: string; image_url: string | null; image_alt: string | null }
...
async function loadPublishedContent(): Promise<ContentSnapshot> {
  ...
  const leadership = ((leadershipResult.data ?? []) as LeaderRow[]).map<Leader>((row) => ({ id: row.id, number: row.number, name: row.name, role: row.role, description: row.description, image: resolveMedia(row.image_url), imageAlt: row.image_alt ?? row.name }))
